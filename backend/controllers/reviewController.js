...
const updateAggregatedRatings = async (targetType, targetId) => {
  ...
  if (targetType === "Farmer") {
    const productQualitySum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.productQuality || 0),
      0,
    );
    const freshnessSum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.freshness || 0),
      0,
    );
    const packagingSum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.packaging || 0),
      0,
    );
  ...
  if (targetType === "Transporter") {
    const timelinessSum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.timeliness || 0),
      0,
    );
    const vehicleConditionSum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.vehicleCondition || 0),
      0,
    );
    const professionalismSum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.professionalism || 0),
      0,
    );
    const count = reviews.length;
  ...
  if (targetType === "Distributor") {
    const paymentReliabilitySum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.paymentReliability || 0),
      0,
    );
    const communicationSum = reviews.reduce(
      (sum, r) => sum + (r.criteria?.communication || 0),
      0,
    );
    const wouldWorkAgainCount = reviews.filter(
      (r) => r.criteria?.wouldWorkAgain === true,
    ).length;
    const count = reviews.length;
...
export const createReview = async (req, res) => {
  try {
      ...
      if (targetType === "Transporter") {
        if (!order.transporter || order.transporter.toString() !== targetId) {
          return res.status(403).json({
            success: false,
            message:
              "You can only review the transporter who delivered this order",
          });
        }
      }
    }
    ...
    if (reviewerRole === "Farmer") {
      await order.populate("product");
      isAuthorized =
        order.product?.farmer?.toString() === reviewerId.toString();
  ...
  try {
    ...
    if (userRole === "Distributor") {
      ...
      return res.status(403).json({
        success: false,
        message:
          "Only Distributors, Farmers, and Transporters can access pending reviews",
      });
    }
    ...
    const pendingOrders = ordersWithReviewStatus.filter((order) =>
      order.reviewableTargets.some((t) => !t.isReviewed),
    );
  ...
  try {
    ...
    if (userRole === "Transporter") {
      ...
      stats = {
        ...
        vehicleCondition:
          aggregated?.transporterSpecific?.averageVehicleCondition || 0,
        professionalism:
          aggregated?.transporterSpecific?.averageProfessionalism || 0,
      };
    }
    ...
    if (userRole === "Distributor") {
      ...
      stats = {
        ...
        paymentReliability:
          aggregated?.distributorSpecific?.averagePaymentReliability || 0,
        communication:
          aggregated?.distributorSpecific?.averageCommunication || 0,
        wouldWorkAgainPercentage:
          aggregated?.distributorSpecific?.wouldWorkAgainPercentage || 0,
      };
    }
