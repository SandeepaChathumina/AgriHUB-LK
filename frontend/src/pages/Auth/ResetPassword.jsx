...
const ResetPassword = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const body = await res.json().catch(() => ({}))
      setStatus({ type: 'success', message: body?.message || 'Password reset successfully.' })
    ...
    <div className="min-h-screen  px-4 py-20 ">
      <div className="mx-auto flex max-w-md flex-col gap-6 rounded-2xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
        <div className="space-y-1 text-center">
          ...
          <h1 className="text-2xl font-semibold text-slate-900">Enter OTP and new password</h1>
          <p className="text-sm text-slate-600">Check your email for the 6-digit code. Enter it with your new password.</p>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="space-y-1">
            <label className="text-sm font-semibold text-slate-800" htmlFor="newPassword">New Password</label>
            <input
              id="newPassword"
              name="newPassword"
              type="password"
