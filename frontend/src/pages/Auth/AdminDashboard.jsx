...
function AdminDashboard() {
  ...
  const fetchPendingReviewsCount = async () => {
    try {
      const res = await fetch('http://localhost:3000/api/reviews/admin/moderation?status=Pending&limit=1', {
        headers: { Authorization: `Bearer ${token}` }
      })
      const data = await res.json()
      if (data.success) {
  ...
  const adminLinks = [
    { title: '👥 Manage Users', to: '/admin/users', icon: '👥', description: 'View and remove users' },
    { title: '🔔 Manage Notifications', to: '/admin/notifications', icon: '🔔', description: 'Send and manage notifications' },
    { title: '⭐ Review Moderation', to: '/admin/reviews', icon: '⭐', description: `Approve or reject reviews (${pendingReviewsCount} pending)` },
    { title: '👤 Manage Profile', to: '/profile', icon: '👤', description: 'Update your profile' },
    { title: '✅ Verify Email', to: '/verify-email', icon: '✅', description: 'Verify your account' },
    { title: '🔑 Reset Password', to: '/reset-password', icon: '🔑', description: 'Change your password' },
  ]
        ...
        <div className="flex flex-col gap-3 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-600">Admin console for verification, user oversight, review moderation, and profile management.</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-lg font-semibold text-slate-900">Admin Links</p>
              </div>
            </div>
