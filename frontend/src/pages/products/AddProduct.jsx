...
const AddProduct = () => {
  ...
  const [gettingRegistrationLocation, setGettingRegistrationLocation] =
    useState(false);
  ...
  const initMap = async () => {
    ...
    try {
      ...
      if (
        ...
        farmerLocation?.coordinates?.lng
      ) {
    ...
    try {
      ...
      if (data.display_name) {
        ...
        if (finalDistrict && finalDistrict !== "") {
          ...
          toast.success(
            "Address found! Please select district manually if needed.",
          );
        }
      }
    } catch (error) {
  ...
  const getRegistrationLocation = () => {
    ...
    if (
      ...
      toast.error(
        "No registration location found. Please update your profile first.",
      );
    ...
    toast.success(
      "Registration location loaded! District: " +
        (farmerLocation.district || "Auto-detected"),
    );
    setGettingRegistrationLocation(false);
  };
  ...
  const handleCoordinatesChange = (coord, value) => {
    ...
    if (
      ...
      const lat =
        coord === "lat" ? numValue : formData.pickupLocation.coordinates.lat;
      const lng =
        coord === "lng" ? numValue : formData.pickupLocation.coordinates.lng;
  ...
  const useFarmerLocation = () => {
    if (farmerLocation?.coordinates?.lat && farmerLocation?.coordinates?.lng) {
      ...
      toast.error(
        "No farmer location found in profile. Please update your profile first.",
      );
    }
  };
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-10 text-center">
            ...
            <p className="text-emerald-700/80 font-medium tracking-wide">
              Showcase your harvest to thousands of potential buyers
            </p>
          </div>
