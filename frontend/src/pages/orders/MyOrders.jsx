...
import { cancelMyOrder, fetchMyOrders, updateMyOrder } from '../../api/orders';
...
const MyOrders = () => {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      const data = await fetchMyOrders(token, pagination.page, pagination.limit);
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-slate-600">Track, update, and manage your placed orders</p>
            </div>
            <Link
              to="/products"
          ...
          <div className="mb-6 grid gap-4 md:grid-cols-5">
            ...
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
              ...
              <p className="text-2xl font-bold text-sky-600">{summary.delivered}</p>
            </div>
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
              <p className="text-xs uppercase text-slate-500">Value (LKR)</p>
              <p className="text-2xl font-bold text-slate-900">{summary.value.toLocaleString()}</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="rounded-2xl bg-white p-12 text-center shadow-sm ring-1 ring-slate-200">
              <p className="text-slate-600">No orders found. Start by placing your first order.</p>
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-slate-700">
                          Qty: <span className="font-semibold">{order.quantity}</span> {order.product?.unit || 'kg'}
                        </p>
                        <p className="text-sm text-slate-700">
                          Amount: <span className="font-semibold">LKR {Number(order.totalPrice || 0).toLocaleString()}</span>
                        </p>
                        <p className="text-sm text-slate-700">
                          Delivery: {order.deliveryAddress?.addressLine || 'N/A'}, {order.deliveryAddress?.city || 'N/A'}
                        </p>
