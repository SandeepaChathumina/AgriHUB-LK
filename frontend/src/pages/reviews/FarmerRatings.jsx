// src/pages/reviews/FarmerRatings.jsx
...
const FarmerRatings = () => {
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/reviews/target/Farmer/${user?.id}?${params}`, {
      ...
      <div className="min-h-screen bg-slate-50 px-4 py-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-600">See what distributors say about your products and service</p>
          </div>

          {/* Stats Cards */}
          {stats && (
            <div className="mb-8 grid gap-6 md:grid-cols-4">
              <div className="rounded-2xl bg-white p-6 text-center shadow-sm ring-1 ring-emerald-200">
                <p className="text-sm text-slate-500">Overall Rating</p>
                <p className="text-5xl font-bold text-emerald-600 mt-2">{stats.averageRating?.toFixed(1) || '0.0'}</p>
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                <p className="text-sm font-semibold text-slate-700 mb-2">Product Quality</p>
                <p className="text-3xl font-bold text-slate-900">{stats.productQuality?.toFixed(1) || '0.0'}/5</p>
                <div className="mt-2">{renderStars(stats.productQuality || 0)}</div>
              </div>

              <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                <p className="text-sm font-semibold text-slate-700 mb-2">Freshness</p>
                <p className="text-3xl font-bold text-slate-900">{stats.freshness?.toFixed(1) || '0.0'}/5</p>
                <div className="mt-2">{renderStars(stats.freshness || 0)}</div>
              </div>

              <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                <p className="text-sm font-semibold text-slate-700 mb-2">Packaging</p>
                <p className="text-3xl font-bold text-slate-900">{stats.packaging?.toFixed(1) || '0.0'}/5</p>
                <div className="mt-2">{renderStars(stats.packaging || 0)}</div>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="rounded-2xl bg-white p-12 text-center shadow-sm ring-1 ring-slate-200">
              ...
              <p className="text-sm text-slate-400">When distributors rate your products, they will appear here.</p>
            </div>
          ) : (
                ...
                <div key={review._id} className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 transition hover:shadow-md">
                  ...
                  {review.response?.text ? (
                    <div className="mt-4 rounded-lg bg-emerald-50 p-4 border-l-4 border-emerald-500">
                      <p className="text-sm font-semibold text-emerald-800 mb-1">Your Response:</p>
