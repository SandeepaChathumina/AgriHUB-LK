...
const EditVehicle = () => {
    ...
    try {
      ...
      setFormData({
        ...
        nextMaintenanceDue: vehicle.nextMaintenanceDue
          ? vehicle.nextMaintenanceDue.split("T")[0]
          : "",
  ...
  const uploadNewImages = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/vehicles/upload-images`,
        {
          method: "POST",
          headers: {
  ...
  const validateSLPlate = (plate) => {
    ...
    return (
      oldFormat.test(normalizedPlate) ||
  ...
  const validateForm = () => {
    ...
    if (
      !formData.loadCapacity.weight.value ||
      formData.loadCapacity.weight.value <= 0
    ) {
                    ...
                    {existingImages.map((image, index) => (
                      <div key={index} className="relative group">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            removeExistingImage(index, image.publicId)
                          }
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Sri Lankan format: 40 Sri 1234, WP-LB-4321, or ABC-1234
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700">
